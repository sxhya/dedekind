...
\begin{prop} \label{prop:Allcock-affine} For a finite simply-laced irreducible root system $\Phi$ and an arbitrary commutative ring $A$
 the group $\St(\Phi, A[X, X\inv])$ is isomorphic to the free product of $\St(\Phi, A)$, the group $X_0(A)$ and the infinite cyclic group $\langle S_0 \rangle$
    ...
    {\allowdisplaybreaks\begin{align}
        ...
        [S_0, w_{\alpha_i}(1)] & = 1; &  \text{$i$ unjoined with $0;$} \label{eq:Allcock-4} \\
        [S_0, x_{\alpha_i}(a)] & = 1, &  \text{$i$ unjoined with $0$; $a \in A;$} \label{eq:Allcock-5-1}\\
...
\begin{prop} \label{prop:rel-poly-Laurent}
    For $\Phi$ and $G$ as above there exists an arrow $\varphi$ making the diagram below commute:
...
\begin{proof}
 ...
 This follows either from a direct computation involving~\cite[Lemma~5.1]{Ma69} and the standard identities for structure constants~\cite[\S~14]{VP},
