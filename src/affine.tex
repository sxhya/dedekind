...
Notice that it is not possible to naively generalize Allcock's presentation from~\cite{A16, A13} to the relative case by replacing $R$ in its statement with an ideal $I$
 since this presentation significantly depends on the unitality of $R$.
A less naive idea to amalgamate rank $2$ relative Steinberg groups over the edges of the Dynkin diagram of $\Phi$ would also not work
 because the resulting group would not even contain root subgroups $X_\alpha(I)$ for all $\alpha \in \Phi$.
However, it turns out to be possible to obtain a weak analogue of Curtis--Tits decomposition for relative Steinberg groups of simply-laced type
...
$z_\alpha(m, a) = x_\alpha(m)^{x_{-\alpha}(a)}$, $\alpha \in \Phi$, $a \in R$, $m \in I$, see~\cite[\S~3.1]{S15}.
