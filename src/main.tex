...
\[ \pi_1^{\mathbb{A}^1}(G)(R) := \mathrm{Hom}_{\mathcal{H}_{\bullet}(k)}(S^1 \wedge \mathrm{Spec}(R)_+, G) = \mathrm{KV}_2^{G}(R) = \K_2^G(R),\]
where $\mathcal{H}_\bullet(k)$ denotes the pointed unstable $\mathbb{A}^1$-homotopy category over $k$.
\end{cor}
