...
\end{equation}
The group $\St(\Phi, R, I)$ can alternatively be defined via generators and relations (cf. \cite[Proposition~6]{S15} or~\cite{V22}).
...
Now for $a \in R^\times$ and $m \in M$ we denote by $\{a, 1+m\}_r$ the element $\{(a; a), (1; 1+m)\}C \in \St(\Phi, R, I)$.
It is clear that the map $1+m \mapsto \{a, 1+m\}_r$ specifies a group homomorphism $(1+M)^\times \to \St(\Phi, R, I)$ and that
 $\mu(\{a, 1+m\}_r) = \{a, 1+m\}$.

For the next lemma we set $R \coloneqq A[X, X^{-1}]$.
\begin{lemma}\label{lem:symbols}
If $A$ is a domain then \[\{A^\times, 1+M\} \cap C(\Phi, A[X, X\inv], M[X, X\inv]) = 1.\]
\end{lemma}
\begin{proof}
Set $F = \mathrm{Frac}(A)$.
...
\[\begin{tikzcd}
 (1+M)^\times \ar[hookrightarrow, rr] \ar[d] &  & F^\times \ar[hookrightarrow, d] \\
  \K_2(\Phi ,A[X^{\pm 1}], M[X^{\pm 1}]) \ar[r] & \K_2(\Phi, A[X^{\pm 1}]) \ar[r] & \K_2(\Phi, F[X^{\pm 1}]).
\end{tikzcd}\]
The right arrow is injective by Hurrelbrink's result (cf.~\cite[Lemma~2.2]{LS20}).
\end{proof}
