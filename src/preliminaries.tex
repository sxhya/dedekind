...
Such digrams for $\Phi$ of type $\rD_\ell$, $\rE_6$ and $\rE_7$ are depicted on Figure 1 (the added root is marked with yellow color).
...
\begin{lemma} $\St(\widetilde{\Phi}, A) \cong \St(\Phi, A[X, X\inv])$.
\end{lemma}
\begin{proof}
    ...
    \end{align}
    It is not hard to check that the map $x_{(\alpha, m)}(a) \mapsto x_\alpha(aX^m)$ is a well-defined homomorphism with the inverse given by
     $x_\alpha(a_{n}X^n + \ldots + a_m X^m) \mapsto \Pi\limits_{i=n}^m x_{(\alpha, i)}(a_i)$, $a_i \in A$, $n \leq m$, $n, m\in \ZZ$.
\end{proof}

We will finish with yet another presentation of affine Steinberg group.
\begin{prop}
    ...
    to the free product of $\St(\Phi, A[X])$ and the cyclic group $\langle S \rangle$ amalgamted over the following list relations:
    \begin{align}
        lol & lol \\
        foo & bar
    \end{align}
...
We conclude this subsection with a weak analogue of Curtis--Tits theorem which holds for relative Steinberg groups.
