...
%Now if $a, b\in R$ satisfy $1 + ab \in R^\times$ one defines the \textit{Dennis--Stein} symbol via the formula
...
%Notice that the above definition of Dennis--Stein symbol is \("\)old-fashioned\("\) in the sense that it agrees with~\cite{DS73} and~\cite{LS20}
% and is different from~\cite[Definition~III.5.11]{Kbook}.
...
%Dennis--Stein symbols are a generalization of Steinberg symbols and the former can be expressed through the latter in the special case when either
%$a$ or $b$ is an invertible element of $R$ (cf. \cite[p.~250]{DS73}):
%\begin{equation} \label{DS-S-relationship}
%\langle a, b \rangle = \{-a, 1+ab\}\text{ for } a, 1+ab\in R^\times,\ \
% \{ u, v \} = \left\langle -u, \tfrac{1 - v}{u} \right\rangle\text{ for } u, v\in R^\times. \end{equation}
