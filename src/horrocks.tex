...
Conversely, we claim that for every pair $(u, \lambda) \in A^\times \times \Lambda$ there exists some element $w_{\lambda, u} \in W(\Phi, A)$ such that
 the following relations hold:
 \begin{equation}\label{eq:w-highestweight-2}  w_{\lambda, u} \cdot v^+ = u v^\lambda,\ \ w_{\lambda, u} \cdot v^\lambda = u^{-1} v^+. \end{equation}
We denote such element by $w_{\lambda, u}$.
\begin{rem}
    In the case $\Phi = \rD_\ell, \rE_6$ (respectively, $\Phi = \rE_7$) every such $w_{\lambda, u}$ can be chosen as a product of at most $2$
     (respectively, $3$) elements $w_{\beta_i}(v)$ for $\beta_i\in\Sigma_k$, where $i\in \{1,2\}$ (respectively, $i\in\{1,2,3\}$) and $\prod v_i = \pm u$ and $\varpi_k + \sum \beta_i = \lambda$.
    Formulae~\eqref{eq:w-highestweight-2} follow from the description of the action of $w_\beta(v)$ on $V$ given by~\cite[Lemma~6]{V00}.
    It is also possible to verify using the method of computing structure constants proposed in~\cite[\S2.2]{V00} that formulae~\eqref{eq:w-highestweight-2} hold on the nose and not just up to signs.
\end{rem}
...
\begin{proof}
 ...
 therefore we can compute the images of the vector $v^{\lambda'}$ under both sides of~\eqref{eq:sides} using~\eqref{eq:w-highestweight-2} as follows:
 \begin{align} \overline{x_2^{-1}} \cdot \overline{x_1^{-1}(0)} \overline{x_1'(0)} \cdot \overline{x_2'} \cdot v^{\lambda'} = x_2^{-1} \cdot {u'}^{-1} \cdot v^+ &= \overline{u}\overline{u'^{-1}} \cdot v^\lambda, \\
               \overline{a(0)} \cdot v^{\lambda'} &= v^{\lambda'} + \sum_{\mu > \lambda'} \overline{a_\mu} v^\mu \end{align}
 for some $a_\mu \in A$.
 From~\eqref{eq:sides} we conclude that $\lambda = \lambda'$ and that $1 - u/u' \in M$ and that $a_\mu \in M$.
 We can find $\alpha \in \Sigma_k$ and $m \in M$ such that the element $x_2' \cdot h_\alpha(1+m) \cdot x_2^{-1}$ stabilizes the highest weight vector and
  hence belongs to $\StW(\Delta_k, A)$ by the same argument as in the proof of~\cref{lem:can-repr}.
 Consequently, we can write $x_2 = c \cdot x_2' \cdot h_\alpha(1+m)$ for some $c \in \StW(\Delta_k, A)$.

 Since $\chi_{\omega, X}$ acts identically on $\StW(\Delta_k, A)$ (cf. the proof of~\cref{lem:winv-chiw}) we obtain from~\eqref{eq:chi-h} and the commutativity of $\StH(\Phi, A[X, X\inv])$ that
\begin{multline*}\label{eq:central-term}
 {x_2}^{-1} \cdot \chi_{\omega, X}(x_2) =
   h_{\alpha}^{-1}(1 + m) \cdot {x_2'}^{-1} \cdot c^{-1} \cdot \chi_{\omega, X}(c \cdot x_2' \cdot h_{\alpha}(1 + m)) = \\
