...
\begin{dfn} \label{sigma-def}
  ...
  \begin{equation} \label{eq:sigma-def} \delta(\omega)(x_1 \cdot x_2, y, z) \mapsto (\sigma(\omega)(x_1)\cdot x_2, x_2^{-1} \cdot \chi_{\omega, X}(x_2 \cdot y), \widetilde{\chi}_{\omega, X}(z)), \end{equation}
  where $x_1 \in N(\omega)$, $x_2 \in \StW(\Phi, A)$,  $y \in B$, $z \in G_M$.
\end{dfn}

In the above definition homomorphisms $\chi_{\omega, X}$ and $\widetilde{\chi}_{\omega, X}$ were defined in~\cref{subsec:weight-automorphisms},
 while $\sigma(\omega)$ has been constructed in~\cref{thm:pairconstr}.
