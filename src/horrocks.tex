...
The key distinction of our proof compared to Tulenbaev's is our use of the economical presentation of the Steinberg group over Laurent polynomial rings (\cref{prop:rel-poly-Laurent}).
This approach enables us to construct only a single ``weight automorphism'' $\delta_{\varpi_k}$, rather than attempting to construct multiple weight automorphisms as Tulenbaev does.
This simplification substantially streamlines the proof, as it eliminates the need to verify, for example, that the weight automorphisms commute with one another —
 an especially challenging task for root systems of types $\rD_\ell$ and $\rE_\ell$.
