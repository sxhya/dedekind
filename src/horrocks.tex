...
\end{lemma}
\begin{proof}
    We can find $\beta \in \Sigma_k^+$ or $\beta \in \Sigma_k^-$ such that $\langle \alpha, \beta \rangle = 1$ consequently we obtain from
    \eqref{eq:sigmadef}, ~\eqref{eq:sigma-def} and~\cite[Lemme~5.1]{Ma69} that there exists $u \in A^\times$ such that
    \begin{multline*}
        [1, 1, z] \rightsquigarrow_{\overline{\delta}^n_{\omega_k}} [1, 1, \widetilde{\chi}^n_{\varpi_k, X}(z)]
        \rightsquigarrow_{h_\beta(u)} [h_\beta(u), 1, \widetilde{\chi}^n_{\varpi_k, X}(z)] = \\
        = [1, h_\beta(u), \widetilde{\chi}^n_{\varpi_k, X}(z)] \rightsquigarrow_{\overline{\delta}^n_{-\omega_k}}
        [1, \{ X^{n \cdot \langle \varpi_k, \beta \rangle}, u \} \cdot h_\beta(u), z].
    \end{multline*}

    \begin{multline*}
    [1, 1, z] \rightsquigarrow_{\overline{\delta}^n_{\omega_k}} [1, 1, \widetilde{\chi}^n_{\varpi_k, X}(z)] \rightsquigarrow_{w^{-1}}
    [w^{-1}, 1, \widetilde{\chi}^n_{\varpi_k, X}(z)] \rightsquigarrow_{x_\alpha(f)} \\
    \rightsquigarrow_{x_\alpha(f)} [x_\alpha(f) \cdot w^{-1}, 1, \widetilde{\chi}^n_{\varpi_k, X}(z)]  \rightsquigarrow_{w}
    [{}^w x_{\alpha}(f), 1, \widetilde{\chi}^n_{\varpi_k, X}(z)] = \\
    = [1, 1, x_{\overline{w} \cdot \alpha}(uf) \cdot \widetilde{\chi}^n_{\varpi_k, X}(z)]  \rightsquigarrow_{\overline{\delta}^n_{-\omega_k}}
    [1, 1, x_{\overline{w} \cdot \alpha}(uX^{-n \langle \omega_k, \overline{w} \cdot \alpha \rangle }f) \cdot z].
    \end{multline*}
\end{proof}
