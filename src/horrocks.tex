...
\begin{proof}{Proof of~\cref{thm:horrocks-k2}}
    ...
    Indeed, for $n, m \geq 0$, $f \in M[X]$, $g \in A[X]$, $\beta \in \Sigma_k^-$ we have that
    \begin{multline*}
        ...
        = [1, x_{-\beta}(g), \widetilde{\chi}^n_{-\varpi_k, X}(z)] \rightsquigarrow_{\delta_{\varpi_k}^{n + m}}
        ...
        = [1, x_{-\beta}(X^{-n-m}g), x_{\beta}(f)^{x_{-\beta}(X^{-n-m}g)} \widetilde{\chi}^m_{\varpi_k, X}(z)] \rightsquigarrow_{\delta^{n + m}_{-\varpi_k}} \\
        ...
        \rightsquigarrow_{ x_{-\beta}(-g) } [1, 1, x_{\beta}(X^{-n-m}f)^{x_{-\beta}(g)} \widetilde{\chi}^n_{-\varpi_k, X}(z)] \rightsquigarrow_{ \delta_{\varpi_k}^n }
        [1, 1, x_{\beta}(X^{-m}f)^{x_{-\beta}(X^{-n}g)} \cdot z].
    \end{multline*}

    On the other hand, for $w \in \StW(\Phi, A)$, $n \geq 0$ and $f \in M[X]$ one has
    \begin{multline*}
       [1, 1, z] \rightsquigarrow_{w} [w, 1, z] = [w_1 \cdot w_{\lambda, u}, 1, z] \rightsquigarrow_{\delta^n_{\omega_k}}
       [w_1 \cdot w_{\lambda, u}, w_{\lambda, u}^{-1} \chi^n_{\varpi_k, X}(w_{\lambda, u} \cdot y), z]
    \end{multline*}
