...
Throughout this section we assume that $\Phi$ is a root system of type $\rD_{\geq 4}$, $\rE_6$ or $\rE_7$
 and $\omega = \pm \varpi_k$, where $k$ is the same as in the statement of \cref{thm:pairconstr}.
...
Conversely, we claim that for every pair $(u, \lambda) \in A^\times \times \Lambda$ there exists some element $w_{\lambda, u} \in W(\Phi, A)$ such that
 \begin{equation}\label{eq:w-highestweight-2} w_{\lambda, u} \cdot v^\lambda = u^{-1} v^+. \end{equation}
%We denote such element by $w_{\lambda, u}$.
\begin{rem}
    If $\lambda = \varpi_k$ the element $w_{\lambda, u}$ can be chosen as $h_\alpha(u)$ for some $\alpha \in \Sigma_k^-$, $u \in A^\times$.
    ...
    \end{itemize}
    Formula~\eqref{eq:w-highestweight-2} follows from the description of the action of $w_\beta(v)$ on $V$ given by~\cite[Lemma~6]{V00}.
\end{rem}
...
\end{lemma}
\begin{proof}
 Choose system of positive roots $\Phi^+'$ to be either $\Phi^+$ or $\Phi^-$ depending on whether $\omega $ is $\varpi_k$ or $-\varpi_k$.
...
\begin{proof}
 ...
 Using~\cite[Lemma~7]{V00} and~\eqref{eq:w-highestweight-2} we can find $\alpha \in \Sigma_k$ and $m \in M$ such that the element $x_2' \cdot h_\alpha(1+m) \cdot x_2^{-1}$ stabilizes the highest weight vector $v^+$.
...
\end{multline*}
 ...
 $ (c^{-1} x_1^{-1} x_1')^{x_2'}  =  h_\alpha(1+m) \cdot a \cdot \mu(b^a) \in (N_\omega)^{x_2'}$
 $a \mu(b^a) h_\alpha^{-1}(1+m) \in {N_\omega}^{x_2'}$

\begin{multline*}
    \delta_{\omega}((a, b, 1) \star (x_1 \cdot x_2, y, z)) = \delta_{\omega}(x_1' x_2', y', z') =
\end{multline*}
